...
void Babel::Ui::MainWindow::onPacketReceived(Babel::Networking::RawPacket packet) {
    if (packet.getPacketType() == Networking::PacketType::PacketRespListFriends) {
        ...
        for (auto &f : resp->getFriends()) {
            auto *friendItem = new FriendItemWidget(f.id, f.username, _chatWidget);
        ...
        if (resp->getOk() == 0)
            return;
        _friendsInnerLayout.addWidget(new FriendItemWidget(resp->getUserId(), resp->getUsername(), _chatWidget));
